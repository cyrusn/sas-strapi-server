...
const students = require('./seedData/student.json')
const { ITEM_CODES, TEACHERS, ABSENT_TYPES, OFFICE_ADMINS, MERIT_DEMERIT_CODES } = require('./seedData/constant')
...
module.exports = {
  async clearDatabase({ strapi }) {
    ...
    await Promise.all([
      ...
      strapi.db.query('api::merit-demerit.merit-demerit').deleteMany(),
    ]).catch(console.error)
  },
      ...
      for (let i = 0; i < n; i++) {
        ...
        switch (type) {
          case "ABSENT_NORMAL_AM":
            ...
            break
          case "ABSENT_NORMAL_HALF_DAY":
            noOfAbsentedLesson = 5
            isLeaveOfAbsence = _.sample([true, true, true, false])
            break
          case "ABSENT_ACTIVITY_DAY":
            noOfAbsentedLesson = 1
            isLeaveOfAbsence = _.sample([true, true, true, false])
            break
